...
for module in $MODULES ;do
    ...
    echo "'$ALL_EXPORTED_SUBS' '$ALL_EXPORTED_VARS'"

    if [ -z "$ALL_EXPORTED_VARS_RX" ] && [ -n "$ALL_EXPORTED_SUBS" ];then
        for f in $(ag -l "use $m;"); do 
            EXPORTED_VALUES=$(ag -s --nocolor --nonumbers  -o "$ALL_EXPORTED_SUBS" $f | sort -u | perl -p -e's/\$/\\\$/;s/^/    /;')
